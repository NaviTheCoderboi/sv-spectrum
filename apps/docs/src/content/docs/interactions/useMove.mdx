...
<div
	style="width: {CONTAINER_SIZE}px; height: {CONTAINER_SIZE}px; background: white; border: 1px solid black; position: relative; touchAction: none;"
>
	<div
		{...moveProps}
		tabindex={0}
		style="width: {BALL_SIZE}px; height: {BALL_SIZE}px; border-radius: 100%; position: absolute; left: {clamp(
			position.x,
		)}px; top: {clamp(position.y)}px; background-color: {color};"
	></div>
</div>
